  ...
  if (port === 10) {
    for (let i = 0; i < bytes.length; ) {
      ...
      if (channel === 0x00 && type === 0xba) {
        ...
        decoded.push({
          variable: "battery_status_life",
          value: 2.5 + (value & 0x7f) * 0.01,
          unit: "V",
        });
      }
      // acceleration alarm status - 1byte unsigned
        ...
        const value = bytes.readUInt8(i++);
        decoded.push({
          variable: "acceleration_alarm",
          value: value === 0x00 ? 0 : 1,
        });
      }
      // acceleration vector = 6bytes signed
        ...
        i += 2;
        decoded.push({
          variable: "acceleration_vector_xaxis",
          value: valuex * 0.001,
          unit: "g",
        });
        decoded.push({
          variable: "acceleration_vector_yaxis",
          value: valuey * 0.001,
          unit: "g",
        });
        decoded.push({
          variable: "acceleration_vector_zaxis",
          value: valuez * 0.001,
          unit: "g",
        });
      }
    ...
    switch (type) {
      case 0x0a:
        ...
        for (let i = 1, j = 1; i < bytes.length; j++) {
          ...
          decoded.push({
            variable: `bd_addr${j}`,
            value: bd_addr.toString(16).toUpperCase().padStart(12, "0"),
          });
      ...
      case 0xb0:
        decoded.push({
          ...
          value: "whitelisting mode, range 0",
        });
        for (let i = 1, j = 1; i < bytes.length; j++) {
          ...
          const rssi = bytes.readInt8(i++);
          decoded.push({
            variable: `bd_addr_lap${j}`,
            value: bd_addr_lap.toString(16).toUpperCase().padStart(6, "0"),
          });
          decoded.push({ variable: `rssi${j}`, value: rssi });
        }
        break;
      case 0xb1:
        decoded.push({
          variable: "message_type",
          value: "whitelisting mode, range 1",
        });
        for (let i = 1, j = 1; i < bytes.length; j++) {
          const bd_addr_lap = bytes.readUIntBE(i, 3);
          i += 3;
      ...
      case 0xb2:
        decoded.push({
          variable: "message_type",
          value: "whitelisting mode, range 2",
        });
        for (let i = 1, j = 1; i < bytes.length; j++) {
          const bd_addr_lap = bytes.readUIntBE(i, 3);
          i += 3;
      ...
      case 0xb3:
        decoded.push({
          variable: "message_type",
          value: "whitelisting mode, range 3",
        });
        for (let i = 1, j = 1; i < bytes.length; j++) {
          const bd_addr_lap = bytes.readUIntBE(i, 3);
          i += 3;
