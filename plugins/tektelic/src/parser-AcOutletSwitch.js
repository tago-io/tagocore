  ...
  for (let x = 1; x < buffer.length; ) {
    switch (buffer[x]) {
      ...
      case 0: // 00
        ...
        data.energy_consumption_meter_status = {
          value: msg_byte.readUInt8(0) === 0 ? "idde" : "active",
          serie,
        };
        x += 1;
        break;
      case 128: // 80
        ...
        if (channel === 0) {
          data.real_power = {
            value: msg_byte.readInt16BE(0) * 0.1,
            unit: "W",
            serie,
          };
        } else {
      ...
      case 129: // 81
        msg_byte = Buffer.from(buffer.slice((x += 1)));
        data.power_factor = {
          value: msg_byte.readUInt8(0) < 254 ? msg_byte.readUInt8(0) : "N/A",
          unit: "W",
          serie,
        };
        x += 1;
      ...
      case 1: // 01
        msg_byte = Buffer.from(buffer.slice((x += 1)));
        data.relay_status = {
          value: msg_byte.readUInt8(0) === 0 ? "Open" : "Closed",
          serie,
        };
        x += 1;
        break;
  ...
  if (everynet_payload) {
    ...
    try {
      vars_to_tago = vars_to_tago.concat(
        toTagoFormat(parsePayload(everynet_payload.value, serie), serie),
      );
    } catch (e) {
