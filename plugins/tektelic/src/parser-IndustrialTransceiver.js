    ...
    for (let i = 0; i < buffer.length; ) {
      ...
      if (channel === 0x00 && type === 0xff) {
        data.push({
          ...
          value: buffer.readInt16BE(i) * 0.01,
          unit: "V",
        });
        i += 2;
      }
          ...
          return [
            {
              ...
              value: "Parser Error: Output 1 value must be 0x00 or 0xFF",
            },
          ];
        }
        data.push({
          variable: "output_1",
          value: value === 0xff ? "Closed" : "Opened",
        });
      }
      // Output 2
        ...
        if (value !== 0x00 && value !== 0xff) {
          return [
            {
              variable: "parser_error",
              value: "Parser Error: Output 2 value must be 0x00 or 0xFF",
            },
          ];
        }
        data.push({
      ...
      // Temperature
        ...
        data.push({
          ...
          value: buffer.readInt16BE(i) * 0.1,
          unit: "°C",
        });
        i += 2;
      }
      // Input 1 State
        ...
        if (value !== 0x00 && value !== 0x01) {
          return [
            {
              variable: "parser_error",
              value: "Parser Error: Input value must be 0x00 or 0x01",
            },
          ];
        }
        data.push({
          variable: "input_1",
          value: value === 0x01 ? "Opened" : "Closed",
        });
      }
      // Input 2
