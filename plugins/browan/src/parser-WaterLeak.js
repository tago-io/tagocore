function Decoder(bytes, port) {
  ...
  if (port === 106) {
    ...
    decoded.push({
      variable: "water_leakage_interrupt",
      value: (bytes[0] >> 4) & 1,
    });
    decoded.push({
      variable: "temperature_changed",
      value: (bytes[0] >> 5) & 1,
    });
    ...
    decoded.push({
      variable: "battery",
      value: (25 + (battery_level & 0x0f)) / 10,
      unit: "V",
    });
    // TEMPERATURE (PCB)
    decoded.push({
      variable: "temperature_pcb",
    ...
    decoded.push({
      variable: "humidity",
      value: bytes.readUInt8(3) & 0x7f,
      unit: "%",
    });
    // TEMPERATURE (ENVIRONMENT)
    decoded.push({
      variable: "temperature_environment",
      value: (bytes.readUInt8(4) & 0x7f) - 32,
      unit: "°C",
    });
  }
