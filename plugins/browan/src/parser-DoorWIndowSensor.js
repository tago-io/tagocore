...
function Decoder(bytes, port) {
  ...
  if (port === 100) {
    // parse status
    decoded.status = {
      value: Number.parseInt(dec2bin(bytes[0]).substr(7, 1), 2),
    };

    // parse battery voltage
    decoded.battery_voltage = {
      value: Number(
        (25 + Number.parseInt(dec2bin(bytes[1]).substr(0, 4), 2)) / 10,
      ).toFixed(1),
      unit: "V",
    };
    ...
    decoded.battery_capacity = {
      value: Number(
        (100 * Number.parseInt(dec2bin(bytes[1]).substr(4), 2)) / 15,
      ).toFixed(1),
      unit: "%",
    };
...
export default async function parserDoorWindowSensor(payload) {
  if (!Array.isArray(payload)) {
    payload = [payload];
  }

  let data = payload.find(
    (x) => x.variable === "data" || x.variable === "payload",
  );
